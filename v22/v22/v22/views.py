...
from v22.settings import HOST, PORT, USER, PASSWORD, DATABASE, MAXID
...
def try_one(request):
    info_id = random.randint(1, MAXID)
    return info(request, info_id)
...
def search(request):
    if request.method == 'GET' and request.is_ajax():
        ...
        sql = '''select `id`, code, title, actor, `type`, `date`, ifnull(score, ''), director, producer, publisher
                 from main_info'''
        sql_count = 'select count(id) from main_info'
        if info_id or code or title or actor or type_ or director or producer or publisher or score:
            sql += ' where'
            sql_count += ' where'
        if score != -1:
